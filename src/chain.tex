...
% FIX
\newcommand{\fix}[1]{{\color{violet}\textbf{#1}}}

\begin{document}
\begin{frontmatter}

\title{Service Chain Placement in SDNs\tnoteref{t1}}
\tnotetext[t1]{
...
\author{Dror Rawitz\fnref{f1}}
\ead{dror.rawitz@biu.ac.il}
\address{Faculty of Engineering, Bar Ilan University, Ramt-Gan 52900, Israel.}
\fntext[f1]{Partially supported by the Israel Science Foundation (grant no.~497/14).}
...
\begin{abstract}
    ...
    general instances in which the service chain passes through a bounded
    number of vertices whose degree is larger than two.
    %
...
\begin{keyword}
approximation algorithms \sep randomized algorithms \sep network function virtualization \sep resource allocation \sep service chaining \sep software defined network.
...
%
Rost and Schmid~\cite{RostSchmid18} considered variant of this
...
\fix{Amiri1 et al.~\cite{AmiriLatin2018, AmiriCCR2018, AmiriIFIP2018} considered the \textsc{Waypoint Routing} problem.
In this problem we are given an edge weighted graph $G=(V, E)$, a source $s \in V$, destination $t \in V$ and a set of waypoints $W \subseteq V$.
The goal is to find a shortest walk from $s$ to $t$ that traverses all the waypoints.
Two variants of this problem exists, namely when the waypoints need to be traversed in a particular or in an arbitrary order.
It is also possible that we have additional capacity consistents, i.e. a limitation on the number of times a walk can go through each edge.
Amiri1 et al. show that (i) There exists a polynomial algorithm for bounded treewidth graph for arbitrary number of waypoints and for general graphs for logarithmically bounded number of waypoints. (ii) This problem is NP-hard in general. (iii) This problem is NP-hard for directed graphs even when there is a single waypoint. (iv) There is a polynomial algorithm for cactus topologies.
}
...
%
\fix{Without loss of generality, we assume that $p(s) = p(t) = p(\sigma) = p(\tau) =
...
\caption{A reduction from a partition instance $a_1, \ldots, a_n$.
  ...
  $v_i^-$ or in $v_i^+$.  In the former case this placement incurs no
...
~,
\end{align*}
where the second inequality is due to the optimally of $f'$ with
respect to $c'$.
\end{proof}
...
\fix{Since we check all the possible values of $c_{max}$ and for each such value we compute an optimal solution with respect to $c'$, then} according to Lemma~\ref{lemma:guess} the best placement is $(1+\eps)$-approximate.  
...
We note that cactus graphs were also considered in~\cite{RostSchmid18}.
...
In this model clients request compound services from the network provider which translate into a service chain that need to be embedded into the physical network.
