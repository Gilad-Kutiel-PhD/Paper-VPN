...
consists of at most $k$ neighborly vertices.  
For example, this can be assumed if all simple paths from $s$ to $t$ contain at most $k$
neighborly vertices.
%
...
DAGs given in Theorem~\ref{thm:fptas}.  
The algorithm finds a $(1-\eps)$-approximate placement with probability $(1-1/e)$, 
and its running time of the algorithm is $O(t(n)k!)$, 
where $t(n)$ is the time it takes to compute a placement when 
the physical network is a DAG.
%
