...
is a DAG.  In addition, based on our FPTAS, we provide a randomized
...
$(1+\eps)$-approximate placement with probability $(1-e^{-1})$ using
$k!$ invocation of the FPTAS.
...
%\infty    & c < 0, \\
\infty    & c(\alpha,v) > c, \\
p(\alpha) & c(\alpha,v) \leq c, \beta = \alpha, \\
\displaystyle
...
neighbors.
%In this section
We assume that there exists an optimal placement whose physical path
consists of at most $k$ neighborly vertices.  For example, this can be
assumed if all simple paths from $s$ to $t$ contain at most $k$
neighborly vertices.
%
We present a randomized algorithm that consists of $k!$ iterations of
the following two phases: an orientation phase that applies a random
...
$(1-\eps)$-approximate placement with probability $(1-1/e)$, and
its running time of the algorithm is $O(t(n)k!)$, where $t(n)$ is the
...
\begin{theorem}
With probability at least $1 - e^{-1}$ the algorithm finds a
$(1+\eps)$-approximate placement after $k!$ iterations.
\end{theorem}
\begin{proof}
...
with respect to $G$.
%
$\pi$ agrees with $\pi'$ with probability $1/k!$, and the probability
that $\pi$ does not agree with $\pi'$ in $k!$ iterations is $1 - (1 -
1/k!)^{k!} \leq 1 - e^{-1}$.
%
\qed
\end{proof}

\paragraph{\textbf{FPT Algorithm}}
Let $k = |N|$, we present an FPT algorithm that runs in $k!$ iterations and is
guaranteed to find the optimal placement.
For each possible permutation of the $k!$ permutations on the $k$ neighborly vertices, 
the algorithm applies the graph orientation procedure as described above,
and then executes the FPTAS for DAGs given in Theorem~\ref{thm:fptas}, 
thus the running time of the algorithm is $O(k!(A + O))$ where $A$ is the running time of the
FPTAS and $O$ is the time it takes to orient an undirected graph.

\begin{theorem}
There is an FPT algorithm that finds a $(1+\eps)$-approximate placement 
in $O(k!(A + O))$ time
\end{theorem} 

\begin{proof}
The running time of the algorithm follows from the discussion above, the correctness of the 
algorithm follows directly from the fact that there is a correct orientation among the $k!$
possible once that corresponds to the optimal placement 
\qed
\end{proof}
