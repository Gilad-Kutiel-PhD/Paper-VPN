...
respectively.
%
We remaind that $\cost^i_{v_i}(\gamma_i,\beta_i)$ is the cost, with respect to
$c_i$, of a minimum cost path from $\gamma_i$ to $\beta_i$ whose total
processing requirement can be handled by $v_i$.  
Consider the following recursive definition:
\[
\ft((\alpha_1,\zeta_1),(\alpha_2,\zeta_2),s,s)
= \cost^1_{s}(\sigma \leadsto \alpha_1) + \cost^2_{s}(\sigma \leadsto \alpha_2) 
\]
otherwise, for $v_1 \neq v_2$, where $v_1$ and
$v_2$ in the same layer, we have
\[
...
Where we take the minimum over all the possible virtual functions and
physical nodes such that:

\begin{itemize}
  \item $\gamma_i \preceq \alpha_i$
  \item $\beta_i \preceq \alpha_i \lor \beta_i = \zeta_i$ 
  \item $(\gamma_i,\beta_i) \in \calE$
  \item $u_1 \neq u_2 \lor u_1 = u_2 = s$
  \item $(u_i,v_i) \in E$
